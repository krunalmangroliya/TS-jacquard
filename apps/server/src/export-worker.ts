...
export function renderDocument({ document, profile, createdAt }: ExportTask): ExportArtifacts {
  ...
  const rendered = render(materialized.geometry, materialized.faces, master.bounds, master.palette, size.widthPx, size.heightPx, document.rules, document.pixelOverrides, master.repeat, master.raster);
  ...
  const sidecar = { app: 'JDM local', document: { id: document.id, name: document.name, kind: document.kind, revision: document.revision, masterId: document.masterId, baseMasterVersion: document.baseMasterVersion }, master: { id: master.id, version: master.version }, sourceInterpretation: master.source.interpretation, sourcePixelAspect: master.bounds.pixelAspect, profile, ...size, palette: master.palette, rules: document.rules, pixelOverrides: document.pixelOverrides, colorsUsed: rendered.report.colorsUsed, colorPixelCounts: rendered.report.colorPixelCounts, changedPixelsByRule: rendered.report.changedPixelsByRule, smallFacesRemoved: rendered.report.smallFacesRemoved, unassignedFaces: rendered.report.unassignedFaces, warnings: [...materialized.warnings, ...size.warnings, ...rendered.report.warnings], bmpSha256: digest(bmp), pngSha256: digest(png), exportedAt: createdAt };
