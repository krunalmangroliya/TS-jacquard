...
interface Props { onOpen: (id: string) => void; onImport: () => void; onImageImport: () => void; onSettings: () => void; refreshKey?: unknown }

export default function Library({ onOpen, onImport, onImageImport, onSettings, refreshKey }: Props) {
  ...
  return <main className="jdm-library jdm-screen">
    <ScreenHeader eyebrow="JACQUARD DESIGN MASTER" title={showArchived?"Archived designs":"Your design library"} description={showArchived?"Restore a design whenever you need it again.":"Keep the original. Make every size from one master."}>
      ...
      <button className="jdm-button" onClick={onImageImport}><Icon name="upload" /> Upload color image</button>
