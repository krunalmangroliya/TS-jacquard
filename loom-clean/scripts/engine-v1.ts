...
export function cleanRaster(
  ...
  const isSourceDetail = (index: number, color: number): boolean => {
    const x = index % w, y = Math.floor(index / w);
    const cx = centerX[x], cy = centerY[y];
    const rx = Math.max(3, Math.min(32, Math.ceil(sx * 1.5)));
    const ry = Math.max(3, Math.min(32, Math.ceil(sy * 1.5)));
    ...
    while (head < tail) {
      ...
      if (xx === left || xx === right || yy === top || yy === bottom) hitsEdge = true;
  ...
  let changedPixels = 0;
  for (let i = 0; i < n; i++) if (pixels[i] !== baseline[i]) changedPixels++;
  ...
  if (outline === null) warnings.push('Select an outline color to enable source-supported repair of one-pixel line gaps.');
