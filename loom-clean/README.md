...
6. Select **Find noisy regions** to inspect dense texture fields across the whole current canvas. Numbered regions propose flattening a mixed fill while retaining the existing palette. **Inspect** zooms to an area; compare **Before**, **Proposed**, and **Changes**, then include or skip individual regions. **Apply selected regions** creates one undoable edit. Discard leaves the canvas untouched. Region detection uses structural and texture analysis; the separate small-model experiment remains under **Small pixel model trial**.
7. Select a palette color, then **Fill connected region**, **One-pixel pencil**, or **Eyedropper**. A region fill uses four-neighbor connectivity; **Replace all occurrences** is an explicit global replacement. Add colors up to 256. Undo/redo retains up to 16 image states, including palette changes.
8. **Export BMP** saves a local copy under **output/exports/** and requests a browser download of the current edited image as an uncompressed 8-bit indexed BMP, with separate horizontal and vertical pixel-density metadata. PNG export is also available. The export endpoint runs on this local machine, not an external service. Palette indices are internal to this app; assign/confirm weaves in NedGraphics after import.
9. **New output** retains the original source and resets the output/editor. Changing settings then reprocessing always starts from that original, never an already-cleaned output.
...
## Whole-region suggestions

The completed examples establish the desired flat fields and retained ornament structure even where the designer changed colors. Recoloring does not make those examples unusable. Reference analysis recovers local alignment and repeat phase before interpreting the differences; see `ai/REFERENCE_REGIONS.md` and `ai/analyze_reference_regions.py`.

Region analysis scans the whole output grid rather than a limited number of model patches. Dense mixed-color neighborhoods seed connected two-ink fields. Existing outline ink and protected colors constrain the proposals; repeated hatching, rings, and lattice structures receive separate checks. Selecting a region previews many pixel replacements together using an existing ink. Reference colors are not copied into the result, and this detector is not the earlier trained speck model. The designer can finish colors after applying the chosen regions.

The canvas shows numbered bounds and zooms to each selected Inspect target. All proposed changes remain separate from editing history until Apply. Intentional shading and small motifs can resemble grain, so each proposed region remains a review decision. Whole-canvas scanning does not mean every possible defect was found or every suggestion is correct.

Run `node node_modules/tsx/dist/cli.mjs scripts/evaluate-regions.ts` to compare all ten frozen V2 source outputs with region proposals and contextual COMPLETE examples. Local artifacts are in `output/region-experiment/`. The report checks input preservation, dimensions, palette, indices, full-grid scanning, selected pixel counts, and BMP round trips. Native detail crops show outlines and hatching alongside broad texture fields. Color equality with COMPLETE is not an accuracy metric.

`ai/analyze_region_results.py` compares the actual V2 source pipeline and region output inside locally aligned COMPLETE fields. It also measures boundary remnants and preserved-pattern crops. Findings and remaining defects are recorded in `ai/REGION_RESULTS.md`; these examples are development checks, not an independent accuracy benchmark.
...
This first model was trained with known artificial gaps and specks in the supplied completed designs, with unchanged negative examples. Entire design IDs are separated: 42482 and 42850 for training, 42973 for validation, and 45842 for held-out testing. It did not use aligned source/final region labels, and consequently does not learn the required whole-field cleanup task. The newer reference study recovers usable region labels through recoloring and layout alignment. Synthetic precision/recall does not establish accuracy on real defects or time saved in NedGraphics. Sigmoid scores are model scores, not probabilities that an edit is correct. The model card in `public/models/loom-tiny-v1.json` records the training split, thresholds, measurements and limits. See `ai/README.md` for reproduction.
...
- The supplied completed sample is 768 × 992 and contains intentional recoloring and structural edits. Locally registered regions are useful cleanup targets for the 768 × 988 sized image; whole-image raw RGB equality is not. Reports do not call singleton reduction accuracy or claim measured labor savings.
